...
public class ProgramGraphWriter {
	...
	public void writeCallGraph(String filename, SetMultimap<Location, Location> callGraph, ControlFlowGraph cfg) {
		// Create dot file
		GraphWriter gwriter = createGraphWriter(filename);
		if (gwriter == null) return;
		...
		try {
			...
			for (Location node : nodes) {
				gwriter.writeNode(node.toString(), node.toString(), getNodeProperties(cfg, node));
			}
